    ...
    static class Decoding {
        ...
        void decode(String s, Node root) {
            ...
            for (Character eachChar : s.toCharArray()) {
                root = eachChar == '0' ? root.left : root.right;
...
void decode(String s, Node root) {
            do {
                s = remainHuffmanString(root, s);
            } while (!s.isEmpty());
        }

        private String remainHuffmanString(Node root, String s) {
            try {
                if (s.charAt(0) == '0' && root.left != null) {
                    return remainHuffmanString(root.left, s.substring(1));
                } else if (s.charAt(0) == '1' && root.right != null) {
                    return remainHuffmanString(root.right, s.substring(1));
                }
